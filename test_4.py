...
class EnvironmentHandler:
    ...
    def determine_working_directory(self):
        """Determines the correct working directory based on the execution context."""
...
def extract_ancestor_data(node, namespaces):
    ...
    ancestors_pythonic = [ancestor for ancestor in node.iterancestors()][::-1]
    df_pythonic = pd.DataFrame([{'Node': ET.QName(ancestor.tag).localname, 'Ancestor': ET.QName(ancestor.tag).localname} for ancestor in ancestors_pythonic])
    df_pythonic.to_csv('ancestors_pythonic.csv', mode='a', index=False, header=False)
    
    #ancestors_original = list(reversed(list(node.iterancestors())))
    #ancestors_original.pop()
    #df_original = pd.DataFrame([{'Node': ET.QName(ancestor.tag).localname, 'Ancestor': ET.QName(ancestor.tag).localname} for ancestor in ancestors_original])
    #df_original.to_csv('ancestors_original.csv', mode='a', index=False, header=False)

    for ancestor in ancestors_pythonic:
...
working_directory = env_handler.working_directory
